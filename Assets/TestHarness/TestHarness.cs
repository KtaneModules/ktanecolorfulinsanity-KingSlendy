		...
		if (startupTime > 0) {
			...
			if (startupTime < 0) {
				...
				foreach (KeyValuePair<KMBombModule, bool> m in modules) {
					if (m.Key.OnActivate != null)
						m.Key.OnActivate();
				}
				foreach (KMNeedyModule m in needyModules) {
					if (m.OnActivate != null)
						m.OnActivate();
				}
	...
	public void SetupEdgework(EdgeworkConfiguration config) {
		...
		if (config == null) {
			...
			for (int a = 0; a < numWidgets; a++) {
				...
				if (r == 0)
					widgets.Add(PortWidget.CreateComponent(PortWidget));
				else if (r == 1)
							...
							for (int i = 0; i < widgetConfig.Count; i++) {
								...
								if (widgetConfig.PortPlateType == PortPlateType.CUSTOM) {
									...
									if (widgetConfig.SerialPort)
										ports.Add("Serial");
									if (widgetConfig.ComponentVideoPort)
										ports.Add("ComponentVideo");
									if (widgetConfig.CompositeVideoPort)
										ports.Add("CompositeVideo");
									if (widgetConfig.HDMIPort)
										ports.Add("HDMI");
									if (widgetConfig.VGAPort)
										ports.Add("VGA");
									if (widgetConfig.USBPort)
										ports.Add("USB");
									if (widgetConfig.PCMCIAPort)
										ports.Add("PCMCIA");
									if (widgetConfig.ACPort)
										ports.Add("AC");
...
public class TestHarness : MonoBehaviour {
	...
	Transform PrepareModuleAnchor(List<List<Transform>> anchors, Transform module, ref int timerFace, Transform parent, bool prepareTwitchPlays = true) {
		...
		if (parent != null)
			module.SetParent(parent, true);
		if (prepareTwitchPlays)
	...
	void PrepareBomb(List<KMBombModule> bombModules, List<KMNeedyModule> needyModules, ref List<Widget> widgets) {
		...
		_timer.OnStopEmergencyLights += delegate {
			_emergencyLightsActivated = false;
		};
		...
		if (sn == null)
			throw new Exception("Could not locate the serial number widget. Cannot continue");
	...
	void Start() {
		...
		foreach (MonoBehaviour s in scripts) {
			...
			foreach (FieldInfo f in fields) {
				...
				fakeInfo.Detonate += delegate {
					if (component.OnBombExploded != null)
						component.OnBombExploded();
				};
				fakeInfo.HandleSolved += delegate {
		...
		for (int i = 0; i < modules.Count; i++) {
			...
			modules[i].OnPass = delegate () {
				...
				if (fakeInfo.modules.All(x => x.Value))
					fakeInfo.Solved();
				return false;
			};
	...
	void Interact() {
		...
		if (currentSelectableArea != null && currentSelectableArea.Selectable.Interact()) {
			...
			if (currentModule != null)
				currentModule.SelectableArea.DeactivateSelectableArea();
